...
public class CacheService extends Service {
    ...
    public void onCreate() {
        ...
        LocalBroadcastManager manager = LocalBroadcastManager.getInstance(this);// 帮助在自己的进程内进行局部广播发送与注册，使用它比直接通过sendBroadcast(Intent)发送系统全局广播有以下几点好处：1    因广播数据在本应用范围内传播，你不用担心隐私数据泄露的问题。2    不用担心别的应用伪造广播，造成安全隐患。3    相比在系统内发送全局广播，它更高效。
    ...
    public void onDestroy() {
        super.onDestroy();
        VolleySingleton.getVolleySingleton(this).getRequestQueue().cancelAll(TAG);// 取消绑定在这个tag对象的上的所有请求
        deleteTimeoutPosts();
    }
